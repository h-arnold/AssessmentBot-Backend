...
3. **Config validation**: any new environment variable must be added to `src/config/env.schema.ts` and documented in `docs/configuration/environment.md`, `.env.example`, and `.test.env.example` (the canonical source of test defaults; any values hard-coded in `test/utils/app-lifecycle.ts` must mirror it).
...
- For binary/image payloads: convert `Buffer` values to base64 strings, and for data URI strings extract the base64 portion after `base64,` so identical underlying bytes canonicalise consistently without double-encoding.
...
- The default in-memory store does **not** support byte-based limits. To achieve size-based eviction **and** TTL:
  ...
  - a `sizeCalculation` function that measures the UTF-8 byte length of the cached response `value` (including metadata and hashed file identifiers stored as part of that response) so the cache counts bytes it actually holds; raw image bytes are never cached and therefore are not part of this calculation.
...
- **Template/version drift**: include prompt template content or a version hash in the cache key so template edits always invalidate cached responses.
- **Image file content**: for file-based images (via the `images` field) always include file content hashes in the cache key so changing image bytes invalidates the cached entry; these high-cost requests must stay cacheable.
...
8. **File-based images**
   - When file-based images arrive via the `images` field, the cache key must include the content hashes of those files (not just their paths or mtimes) so edits to the files result in cache misses.
...
4. **API key scoping (optional)**
   - When global caching is disabled or API-key scoping is required, ensure the same DTO from different API keys yields distinct cache keys (for example by prefixing with a hashed API key identifier).
...
4. **API key isolation (optional)**
   - When global caching is disabled or a future requirement mandates per-key isolation, the same payload from different API keys must produce distinct cache entries.
...
3. **Buffer vs string equivalence**
   - Align canonicalisation tests with the runtime prompt path. Confirm identical underlying binary content yields the same cache key whether supplied as a `Buffer` or a data URI string, and that genuinely different content causes a cache miss.
...
6. **Large payload cache poisoning attempt**
...
9. **Template/version change invalidation**
   - Modify prompt templates between requests and ensure cache keys change so that template edits invalidate existing entries.
10. **File-based image invalidation**

- Change an image file on disk between requests and ensure cache misses occur because the content hash in the cache key has changed.
...
2. **Hash must be HMAC-SHA256-based** and use a secret from environment variables.
...
1. **Buffers vs strings** in IMAGE task fields (`reference`, `template`, `studentResponse`) must canonicalise consistently (the DTO enforces uniform typing for those fields while the `images` array always uses string paths).
...
4. **TTL less than one minute** (including zero or negative) must be rejected to avoid indefinite retention; use a feature flag or skip the interceptor to disable caching when needed.
...
- `.env.example`
  -- `.test.env.example` (test defaults; keep any hard-coded values in `test/utils/app-lifecycle.ts` synced with it)
...
3. **Create cache key utility** with canonicalisation, HMAC, and explicit file-based image content hashing in the cache key.
...
- File-based images (via the `images` field) are content-hashed and included in the cache key so content edits always invalidate cached entries.
